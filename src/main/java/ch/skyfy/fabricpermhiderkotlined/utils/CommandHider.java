    ...
    public static <S extends CommandSource> TriState canUse(S source, RootCommandNode<S> root, CommandNode<S> node, String prefix) {
        ...
        for (List<CommandNode<S>> ancestry : ancestries) {
            ...
            while (ancestryIter.hasPrevious()) {
                ...
                if (ancestor.getCommand() != null) {
                    System.out.println("ancestor.getCommand().getClass().getPackageName()" + ancestor.getCommand().getClass().getPackage().getName());
                    System.out.println("ancestor.getCommand().getClass().getPackageName()" + ancestor.getCommand().getClass().getPackageName());

                    FabricLoaderImpl.INSTANCE.getModsInternal().forEach(modContainer -> {
//                        modContainer.getMetadata().getLanguageAdapterDefinitions().forEach((s, s2) -> {
//                            System.out.println("s1: " +s);
//                            System.out.println("s2: " +s2);
//                        });
                        modContainer.getMetadata().getEntrypointKeys().forEach(s -> {
                            System.out.println("entryppnt key: " + s);
                            var entry= modContainer.getMetadata().getEntrypoints(s);
                            entry.forEach(entrypointMetadata -> {
//                                entrypointMetadata.getValue()

                            });
                        });
                        modContainer.getCodeSourcePaths().forEach(path -> {
//                            System.out.println("path12.: "+ path);
                    ...
                    FabricLoader.getInstance().getAllMods().forEach(modContainer -> {
                        ...
                        try {
//                            System.out.println("1: " + modContainer.getOrigin().getKind().getDeclaringClass().getPackageName());
//                            System.out.println("2: " + modContainer.getOrigin().getParentSubLocation());

//                            modContainer.getMetadata().getCustomValues().forEach((s, customValue) -> {
//                                System.out.println("s: " + s);
//                                System.out.println("value: " + customValue.toString());
//                            });
//                            modContainer.getRootPaths().forEach(path -> System.out.println("paht2 : " + path.toString()));
//                            modContainer.getOrigin().getPaths().forEach(path -> System.out.println("path: " + path.toString()));
